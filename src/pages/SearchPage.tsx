...
export default function SearchPage() {
  ...
  const saveRecentSearch = (term: string) => {
    const updated = [term, ...recentSearches.filter(s => s.toLowerCase() !== term.toLowerCase())]
      ...
      <div className="sticky top-0 bg-background z-10 px-4 pt-4 pb-3 safe-top border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1">
            ...
            <Input
              autoFocus
              placeholder="Search coffee, latte, espresso..."
              value={query}
            ...
            <div className="grid grid-cols-2 gap-3">
              {filteredProducts.map((product, index) => (
                <motion.div
                  ...
                  className="bg-card rounded-2xl p-3 shadow-card cursor-pointer active:scale-95 transition-transform"
                >
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-xs text-muted-foreground">4.5 • 250+</span>
                  </div>
        ...
        {query.trim() && filteredProducts.length === 0 && (
          ...
          >
            ...
            <p className="text-muted-foreground text-sm mt-1 text-center">
              We couldn't find "{query}".<br />Try a different search term.
            </p>
          </motion.div>
        )}
              ...
              >
                ...
                <div className="space-y-1">
                  {recentSearches.map((term) => (
                    <div
                      key={term}
                      className="flex items-center justify-between py-2.5 px-3 rounded-xl hover:bg-secondary transition-colors"
                    >
                      <button
              ...
              <div className="flex flex-wrap gap-2">
                {POPULAR_SEARCHES.map((term) => (
                  <button
                    key={term}
                    onClick={() => handleSearch(term)}
                    className="px-4 py-2 bg-secondary rounded-full text-sm font-medium hover:bg-accent hover:text-accent-foreground transition-colors active:scale-95"
                  >
                    {term}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-4 gap-3">
                {categories.slice(0, 4).map((cat) => (
                  <button
                    ...
                    className="flex flex-col items-center p-3 bg-secondary rounded-xl hover:bg-accent/10 transition-colors active:scale-95"
                  >
