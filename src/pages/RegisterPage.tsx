...
export default function RegisterPage() {
  ...
  const handleGoogleRegister = async () => {
    ...
    if (error) toast({ title: t('toast.registerFailed'), description: 'Google signup failed', variant: 'destructive' });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success && result.message === 'verification_needed') {
      ...
      toast({ title: t('toast.welcome'), description: t('toast.accountCreated') });
      navigate('/home');
    } else {
      toast({ title: t('toast.registerFailed'), description: result.message || 'Error', variant: 'destructive' });
    }
  };
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }} className="flex-1 px-6 -mt-4 pb-6">
        <div className="bg-white rounded-3xl p-6 shadow-xl" style={{ boxShadow: '0 10px 40px -10px rgba(43, 31, 22, 0.15)' }}>
          <div className="text-center mb-5">
            <h1 className="text-2xl font-display font-bold mb-1" style={{ color: '#2B1F16' }}>{t('register.title')}</h1>
            <p className="text-sm" style={{ color: '#6B5C52' }}>{t('register.subtitle')}</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium" style={{ color: '#2B1F16' }}>{t('register.fullName')}</label>
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5" style={{ color: '#9C8B80' }} />
                <Input type="text" placeholder={t('register.fullNamePlaceholder')} value={name} onChange={(e) => setName(e.target.value)} className="pl-12 h-12 rounded-xl border-2" style={{ borderColor: '#E8DED6', backgroundColor: '#FDFBF9' }} />
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium" style={{ color: '#2B1F16' }}>{t('register.email')}</label>
              <div className="relative">
                <Mail className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5" style={{ color: '#9C8B80' }} />
                <Input type="email" placeholder={t('register.emailPlaceholder')} value={email} onChange={(e) => setEmail(e.target.value)} className="pl-12 h-12 rounded-xl border-2" style={{ borderColor: '#E8DED6', backgroundColor: '#FDFBF9' }} />
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium" style={{ color: '#2B1F16' }}>{t('register.password')}</label>
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5" style={{ color: '#9C8B80' }} />
                <Input type={showPassword ? 'text' : 'password'} placeholder={t('register.passwordPlaceholder')} value={password} onChange={(e) => setPassword(e.target.value)} className="pl-12 pr-12 h-12 rounded-xl border-2" style={{ borderColor: '#E8DED6', backgroundColor: '#FDFBF9' }} />
            ...
            <Button type="submit" disabled={isLoading} className="w-full h-12 rounded-full text-base font-semibold mt-2" style={{ backgroundColor: '#B87333', color: 'white', boxShadow: '0 8px 24px -8px rgba(184, 115, 51, 0.5)' }}>
              {isLoading ? (<><Loader2 className="h-5 w-5 animate-spin mr-2" />{t('register.submitting')}</>) : (t('register.submit'))}
            </Button>

            <p className="text-center text-xs pt-2" style={{ color: '#9C8B80' }}>
              {t('register.terms')}{' '}
              <button type="button" className="font-medium" style={{ color: '#B87333' }}>{t('register.termsLink')}</button>{' '}{t('register.and')}{' '}
              <button type="button" className="font-medium" style={{ color: '#B87333' }}>{t('register.privacyLink')}</button>
            </p>
          </form>

          <div className="flex items-center gap-4 my-5">
            <div className="flex-1 h-px" style={{ backgroundColor: '#E8DED6' }} />
            <span className="text-sm" style={{ color: '#9C8B80' }}>{t('register.or')}</span>
            <div className="flex-1 h-px" style={{ backgroundColor: '#E8DED6' }} />
          </div>

          <Button type="button" variant="outline" className="w-full h-12 rounded-xl border-2 font-medium" style={{ borderColor: '#E8DED6', color: '#2B1F16', backgroundColor: 'white' }} onClick={handleGoogleRegister}>
        ...
        <div className="text-center py-5">
          <p className="text-sm" style={{ color: '#6B5C52' }}>
            ...
            <Link to="/login" className="font-semibold" style={{ color: '#B87333' }}>{t('register.login')}</Link>
          </p>
        </div>
      </motion.div>
    </div>
